...
const createMenu = (menuObj) => ({
    ...
    pay() {
      ...
      return parseFloat((this.consumption
      .reduce((total, item) => {
        if (Object.keys(food).includes(item)) return total + (food[item] * 1.1);
        return total + (drink[item] * 1.1);
      }, 0)).toFixed(2));      
    },
  });
...
console.log((2.887678).toFixed(2));
